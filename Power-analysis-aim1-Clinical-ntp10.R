...
# 2. The clinical endpoint is a survival outcome
#    For sample size purpose, an ad-hoc approach is adopted
#      following the approach of the accelerated failure time model as used in
#      (Liu, J., Huang, J., Zhang, Y., Lan, Q., Rothman, N., Zheng, T., & Ma, S. (2013).
#       Identification of gene–environment interactions in cancer studies 
#       using penalization. Genomics, 102(4), 189-194.)
#    For this approach, after some transformation, turns the model into 
#      a linear regression framework, such that we can use the same set of codes
#      as used for EQ-5D-5L - with the effective sample size reduced due to censoring.
########################################################################################
...
# input parameters
ntp = 10          # number of true positives
nrep = 100        # number of replicates

# dimension from LIVE BETTER Trial
n = 640*0.7       # sample size and assume 30% censoring rate 
...
saveRDS(object = list(coefList=coefList,rList=rList,
        ...
        file = paste("PowerAnalysis-aim1-Clinical-ntp",ntp,".RDS",sep=""))