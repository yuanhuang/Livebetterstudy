...
data_setting_ntp10_mat <- matrix(c(
  c(0.4, 0.5,-0.55, -0.45),
  c(0.5, 0.6,-0.65, -0.55),
  c(1, 1.1, -1.15, -1.05)), nrow = 3, byrow = TRUE)

data_setting_ntp10 <- data.frame(data_setting_ntp10_mat)
colnames(data_setting_ntp10) <- paste0("range", 1:4)

for (i in 1:nrow(data_setting_ntp10)) {
  mtp <- i
  range1 <- as.numeric(data_setting_ntp10[i, c(1, 2)])
  range2 <- as.numeric(data_setting_ntp10[i, c(3, 4)])
